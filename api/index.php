...
$app->post('/user/add', function(Request $req, Response $res)
{
    ...
    {    
        ...
        $stmt=$this->db->prepare("INSERT INTO users(email,password,username,roles) VALUES(2,:email,:password,:username)");
        ...
        $id = $this->db->lastInsertId();
            ...
            else
            {
                ...
                return $this->response->withJson(array('msg' => 'No se ha podido añadir el usuario :( '));
            }
        }
...
$app->delete('/user/del/{id}', function(Request $req, Response $res, $args)
{
    ...
    if(empty($result))
    {
        ...
        return $this->response->withJson(array('msg' => 'Usuario eliminado correctamente.'));
    }
    else
    {
        //Falla, devolvemos un mensaje avisando del problema
        return $this->response->withJson(array('msg' => 'No se ha podido borrar el usuario.'));
    }
});
